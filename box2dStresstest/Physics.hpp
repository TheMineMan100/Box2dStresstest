...
namespace physics
{ //all functions are inline to avoid redeclaration
	...
	inline Block createBox(b2World& world, float x, float y, float width, float height, b2BodyType type = b2_dynamicBody)
	{
		...
		float LO = -5.0;
		float HI = 5.0;
		float randomVx = LO + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (HI - LO)));
		float randomVy = LO + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (HI - LO)));

		body->ApplyLinearImpulse(b2Vec2(randomVx, randomVy), body->GetWorldCenter(), false);

		return body;
	}

	inline Block createCircle(b2World& world, float x, float y, float r, float vx = 0, float vy = 0, b2BodyType type = b2_dynamicBody)
	{
		b2BodyDef bodyDef;
